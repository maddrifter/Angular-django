        ...
        onUserLoaded = (val) => {
           ...
           */
            if (this.projectSlug != null) {
                this.subscribeProject([this.projectSlug]);
            }
        }

        subscribeProject(projectSlugs:Array<string>){
            var access = this.userService.access(projectSlugs[0]);
            if (access == null) {
                return;
            }

            this.PUBNUB_SUB_KEY = access.subscribeKey;
            this.PUBNUB_PUB_KEY = access.publishKey;
            this.UUID = access.uuid;

            _.forEach(projectSlugs, (projectSlug) => {
                var p = this.userService.me.project_access[projectSlug];
            ...
            this.joinProject();
            console.log(this.PubNub.ngListChannels())
        }

        unSubscribeProject(projectSlugs:Array<string>){
            _.forEach(projectSlugs, (projectSlug) => {
                var p = this.userService.me.project_access[projectSlug];
                var i = this.channelName.indexOf(p.channel);
                if(i != -1){
