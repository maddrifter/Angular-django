...
; (function ($, window, document, undefined) {
  ...
  $.extend(Plugin.prototype, {
    ...
    connectSetup: function (pathConfig) {
      if (pathConfig.hasOwnProperty("start") && pathConfig.hasOwnProperty("end")) {
        ...
        if ($start.length && $end.length) {
          ...
          var strokeDasharray = pathConfig.hasOwnProperty("strokeDasharray") ? pathConfig.strokeDasharray : this.settings.strokeDasharray;
    ...
    connectElements: function (pathData) {
      ...
      if (orientation != "vertical" && orientation != "horizontal" && orientation != "curve") {
    ...
    drawPath: function ($path, offset, orientation, startX, startY, endX, endY) {
      ...
      if (this.$svg.attr("height") < (Math.max(startY, endY) + stroke+100)) this.$svg.attr("height", (Math.max(startY, endY) + stroke+100));
      ...
      if (orientation == "vertical") {
        ...
        // Curve
        var mid = Math.floor((startX + endX)/2);
        $path.attr("d", "M" + startX + " " + startY +
          " C " + (mid) + " " + (startY+80) + " " + mid + " " + (endY+80) + " " + endX + " " + endY);
      }
    },
